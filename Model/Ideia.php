...
class Ideia
{
    ...
    public static function criar(int $userId, string $titulo, string $descricao, string $categoria, string $prioridade, string $status): int
    {
        ...
        $stmt->execute(['user_id' => $userId, 'titulo' => $titulo, 'descricao' => $descricao, 'categoria' => $categoria, 'prioridade' => $prioridade, 'status' => $status]);
    ...
    {
        $pdo = Conexao::get();
        $stmt = $pdo->prepare('UPDATE ideas SET titulo = :titulo, descricao = :descricao, categoria = :categoria, prioridade = :prioridade, status = :status WHERE id = :id AND user_id = :user_id');
        return $stmt->execute(['titulo' => $titulo, 'descricao' => $descricao, 'categoria' => $categoria, 'prioridade' => $prioridade, 'status' => $status, 'id' => $id, 'user_id' => $userId]);
    }
    ...
    {
        $pdo = Conexao::get();
        $stmt = $pdo->prepare('SELECT status, COUNT(*) as total FROM ideas WHERE user_id = :user_id GROUP BY status');
        $stmt->execute(['user_id' => $userId]);
